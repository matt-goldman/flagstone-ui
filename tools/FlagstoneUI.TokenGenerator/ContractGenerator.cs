...
public class ContractGenerator
{
	...
	public async Task<string> GenerateMinimalContractAsync(string sourceDirectory)
	{
		...
		var contract = new Dictionary<string, object?>
		{
			...
			["layer"] = "theme",
			["stylingSurface"] = new Dictionary<string, object?>
			{
				["controls"] = controls.ToDictionary(
					...
					kvp => (object)new Dictionary<string, object?>
					{
						...
						["styledProperties"] = kvp.Value.StyledProperties.Select(p => new Dictionary<string, object?>
						{
							["name"] = p.Name,
							["type"] = p.Type,
			...
			["requiredStyles"] = controls.ToDictionary(
				kvp => kvp.Key,
				kvp => (object)new Dictionary<string, object?> { ["implicit"] = true }
			),
		...
		string? extends = "minimal")
	{
		...
		var contract = new Dictionary<string, object?>
		{
			...
			["name"] = contractName,
			["version"] = "1.0.0",
			["description"] = $"{char.ToUpper(contractName[0])}{contractName[1..]} design system contract. Extends {extends} with named style variants.",
			["layer"] = "design-system",
			["extends"] = extends,
	...
	private Dictionary<string, object> ExtractTokenSchema(string tokensPath)
	{
		...
		foreach (var (key, colorData) in doc.Descendants(mauiNs + "Color")
			.Select(element => (
				Key: element.Attribute(xNs + "Key")?.Value,
				Element: element
			))
			.Where(x => x.Key != null && x.Key.StartsWith("Color.", StringComparison.Ordinal))
			.Select(x => (
				Key: x.Key!,
				Data: new
				{
					Type = "color",
					Category = GetColorCategory(x.Key!),
					Purpose = GetTokenPurpose(x.Key!)
				}
			)))
		{
			schema[key] = colorData;
		}

		// Parse x:Double elements
		foreach (var (key, doubleData) in doc.Descendants(xNs + "Double")
			.Select(element => element.Attribute(xNs + "Key")?.Value)
			.Where(key => key != null)
			.Select(key =>
			{
		...
		{
			schema[key] = doubleData;
		}

		// Parse x:Int32 elements
		foreach (var (key, intData) in doc.Descendants(xNs + "Int32")
			.Select(element => element.Attribute(xNs + "Key")?.Value)
			.Where(key => key != null)
			.Select(key =>
			{
