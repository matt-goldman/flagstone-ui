...
public class ControlSurfaceExtractor
{
	...
	public Dictionary<string, ControlSurface> ExtractFromDirectory(string controlsPath)
	{
		...
		foreach (var surface in controlFiles.Select(ExtractFromFile).Where(s => s != null))
		{
			...
			if (results.TryGetValue(key, out var existing))
			{
				...
				foreach (var prop in surface.StyledProperties.Where(prop => !existing.StyledProperties.Any(p => p.Name == prop.Name)))
				{
	...
	private static void AddInheritedButtonProperties(List<StyledProperty> properties)
	{
		...
		foreach (var prop in inheritedProps.Where(prop => !properties.Any(p => p.Name == prop.Name)))
		{
			properties.Add(prop);
		}
	}
