...
- [x] **Tighten the `FsTabContext` projection.** `FsShell.RebuildTabs` now subscribes to `PropertyChanged` on each `ShellContent`; title/icon edits at runtime flow through to the bar without a full rebuild.
...
- [x] **`FlyoutItem` handling.** Decided: `FlyoutItem` children are skipped in `RebuildTabs` and do not contribute to the tab bar. Flyout chrome is out of scope for V1; decision enforced with a `is FlyoutItem` guard.
