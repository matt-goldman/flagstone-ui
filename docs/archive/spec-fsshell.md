...
- **A *distinct* custom tab bar per `ShellItem`.** `FsShell.TabBar` (and `TabBarItemTemplate`) apply uniformly to whatever item is active — one bar look for the whole app. The bar is correctly *scoped* per item (each item projects its own sections; see "Per-`ShellItem` bar scoping" in §A), but you cannot assign a different bar *instance* or template to each item. This is deliberate: an app has one tab-bar identity, and most apps do not mix flyout and tab-bar navigation the way Shell permits. Supporting per-item custom bars would require turning `TabBar` into a template/factory; it is explicitly **deferred to a post-V1 release** and should not be reopened for V1. The default `FsTabBar` and a single consumer-supplied custom bar both already work per item.
...
> **Bootstrap complete.** The scaffold commit delivered the full cross-platform API skeleton — `FsShell`, `FsTabBar`, `FsTabContext`, `IFsTabBar`, `ITabTransitionAnimator`, `FsTabTransitionContext`, `UseFlagstoneUI()` registration, and stub `FsShellRenderer` partials on every supported platform. The three design decisions below are resolved and codified in that code; the remaining items are implementation tasks tracked here in the spec rather than in a separate design document.

- [x] **Resolve open question: bar selection model.** Decided: `SelectedRoute` (string) is the `IFsTabBar` contract; `SelectedItem` (`FsTabContext`) is a convenience on the concrete `FsTabBar` only and may be promoted to the interface later without breaking changes. Codified in `IFsTabBar` and `FsTabBar`.
- [x] **Resolve open question: VSM on the default template.** Decided: `FsTabBar` pumps `CommonStates` (driven by `IsEnabled`) and `SelectionStates` (`Selected` / `Unselected`, driven by `IsSelected`) on each template-instance root. Implementation pending in the render/tap path; the decision is settled.
- [x] **Resolve open question: top tabs.** Decided: V1 does not visually replace the top-tab strip when a `Tab` contains multiple `ShellContent`; stock Shell rendering applies. Flagged in docs (Non-goals section).
- [x] **Tighten the `FsTabContext` projection.** `FsShell.RebuildTabs` subscribes to `PropertyChanged` on each `ShellSection`; title/icon (and `CurrentItem`-driven fallback) edits at runtime flow through to the bar without a full rebuild.
- [x] **Refine the items rebuild.** `RebuildTabs` diffs the `ObservableCollection<FsTabContext>` in-place via `SyncTabs`; existing `FsTabContext` instances are preserved across rebuilds so BindableLayout does not recreate template instances.
- [x] **Per-`ShellItem` bar scoping (revised).** **Superseded earlier decision** to flatten all `ShellContent` into a single global bar and skip `FlyoutItem`s. The bar is now scoped to the active `ShellItem` exactly as Shell's native bottom bar is: `RebuildTabs` projects `CurrentItem.Items` (the active item's `ShellSection`s) into `FsTabContext`s, and re-projects when the active item changes (e.g. via the flyout). This preserves Shell's full hierarchy — flyout chrome and the top-tab strip keep stock rendering (V1 non-goals), while every item that has bottom tabs gets the FlagstoneUI bar. Tab selection maps to `CurrentItem.CurrentItem = section` (direct section activation, matching stock). The bar auto-hides when the active item has fewer than two sections (matching stock's "no bottom bar for a single section"). Verified on an emulator with a flyout → per-item tabs → top tabs shell. Decision recorded in [ADR012_1 — Per-`ShellItem` Tab Bar Scoping](../decisions/adr012_1-fsshell-per-item-bar-scoping-addendum.md).
- [x] **`Shell.SetTabBarIsVisible` bridging at the cross-platform layer.** `FsShell.BridgeTabBarVisibility` reads `Shell.GetTabBarIsVisible(CurrentPage)` on every navigation and sets `TabBar.IsVisible` accordingly.
- [x] **`HideTabBarOnKeyboard` property.** Added as an instance `BindableProperty` on `FsShell` (default `true`). Per-platform renderers read this to gate keyboard-avoidance behaviour.
...
- [x] **Public API review.** `RebuildTabs` promoted to `protected virtual` (subclassable). `FsTabBar.OnTabTapped` changed from `protected internal virtual` to `protected virtual`. `FsTabBar` now subscribes to `INotifyCollectionChanged` on `ItemsSource` and to `PropertyChanged` on each `FsTabContext`, pumping `VisualStateManager` `SelectionStates` (`Selected` / `Unselected`) and `CommonStates` (`Normal` / `Disabled`) on template-instance roots.
...
- [~] **Android — `FsShellRenderer`.** Core hosting implemented and verified on an emulator. Implemented as `FsShellItemRenderer : ShellItemRenderer` (override `CreateShellItemRenderer`), hiding the native bar and appending the bar's `ContentViewGroup` to the active `ShellItem`'s outer `LinearLayout`. Verified across all three navigation levels coexisting: a `<FlyoutItem>` with two `<Tab>` sections (styled bottom bar) where one section has two `ShellContent`s (stock top-tab strip), plus a second single-section flyout item (stock, no bottom bar), with stock flyout drawer switching between them.
  ...
  - [x] Host the FlagstoneUI bar via the bar's `ContentViewGroup` (`bar.ToPlatform(mauiContext)`), appended as the bottom sibling in the outer `LinearLayout`. **Only intervenes when the active item has more than one section** (i.e. when stock would draw a bottom bar); single-section items are left entirely on stock rendering. The shared bar is released in `OnDestroyView` (guarded against a newer fragment having already re-parented it) so it survives re-hosting across item switches.
  - [x] Respect Android system bars / gesture navigation insets (`WindowInsetsCompat`) — bottom padding from system-bars + IME insets.
  - [~] Bridge `Shell.SetTabBarIsVisible` — handled at the cross-platform layer (`FsShell.BridgeTabBarVisibility` toggles `TabBar.IsVisible`, also hiding when the active item has <2 sections); not yet verified on-device.
  ...
  - **Resolved (per-`ShellItem` bars):** the bar is scoped to the active `ShellItem`, so flyout items with their own bottom tabs are fully supported — not just the single-`<TabBar>` case. See the revised "Per-`ShellItem` bar scoping" item under §A. A *distinct* custom bar instance/template per item is intentionally out of scope for V1 (see Non-goals); `FsShell.TabBar` applies one bar identity app-wide.
